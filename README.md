...
Phase 4 is implemented: `btk privkey`, `btk pubkey`, `btk address`, and `btk node`. Next is `btk balance`. Later: `config`. Help and version are `--help` / `--version` (no `help` or `version` command).
